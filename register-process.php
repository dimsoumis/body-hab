...
if (empty($registrantEmail) OR empty($registrantPassword) OR empty($registrantAge) OR empty($registrantGender) OR empty($registrantStatus)) {	
	...
	if (filter_var($registrantEmail, FILTER_VALIDATE_EMAIL)) {
		...
		$select2 = mysqli_query($conn, "SELECT * FROM therapists WHERE email = '".$registrantEmail."'");
if(mysqli_num_rows($select)) {	
	...
	echo ("<script LANGUAGE='JavaScript'>
    window.alert('This email is used for the account of a therapist! Please use a different email!');
    window.history.back();
    </script>");
} else {
	...
	$sql = "INSERT INTO users (id, firstname, lastname, email, password, age, gender, status)
VALUES ('0', '$registrantFirstName', '$registrantLastName', '$registrantEmail', '$registrantPassword', '$registrantAge', '$registrantGender', '$registrantStatus')";
